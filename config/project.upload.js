...
// 完整的上传目录
let entireBasePath = './public_html/act2/2018/01' + appName;
...
setTimeout(function () {
  ...
  ftp.on('ready', function () {
    log(chalk.green('连接 ftp 成功'));
    createProjectBaseDir(entireBasePath)
      .then(() => {
        log(chalk.green('准备开始上传文件...'));
        readUploadDir(curEnvDir, entireBasePath);
      });
  });

  ftp.on('error', function (err) {
    log(chalk.red('连接 ftp 失败'));
  });
